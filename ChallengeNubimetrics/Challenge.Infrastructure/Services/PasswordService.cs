...
namespace Challenge.Infrastructure.Services
{
    public class PasswordService : IPasswordService
    {
        ...
        public bool Check(string hash, string password)
        {
            ...
            if (parts.Length != 3)
            {
                throw new AppException("Unexpected hash format");
            }
            var iterations = Convert.ToInt32(parts[0]);
            var salt = Convert.FromBase64String(parts[1]);
            var key = Convert.FromBase64String(parts[2]);

            using (var algoritm = new Rfc2898DeriveBytes(password, salt, iterations))
            {
        ...
        {
            using (var algoritm = new Rfc2898DeriveBytes(password, _options.SaltSize, _options.Iterations))
            {
